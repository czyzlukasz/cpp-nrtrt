...
TEST_F(WorkerTest, CheckColorFromFront){
    ...
    const Pixel<> result = worker.getColorAtRay(front);
    const Pixel<> whitePixel{255, 255, 255, 255};
    EXPECT_EQ(result, whitePixel);
}

TEST_F(WorkerTest, CheckColorFromBack){
    const Ray back{{1, 1, -2}, {0, 0 , 1}};
    const Pixel<> result = worker.getColorAtRay(back);
    const Pixel<> whitePixel{255, 255, 255, 255};
    EXPECT_EQ(result, whitePixel);
}
...
TEST_F(WorkerTest, CheckColorFromAngle){
    ...
    std::array<AveragePixel, 160000> container;
    ...
    const Pixel<> grayPixel{static_cast<sf::Uint8>(255.f * factor),
                          ...
                          static_cast<sf::Uint8>(255.f * factor),
                          255};
    EXPECT_EQ(container.at(0).getPixel(), grayPixel);
}
...
TEST_F(WorkerTest, CheckIfRaysPenetrate){
    ...
    for(uint idx = 0; idx < 1000; ++idx){
        worker.processRays(container);
        ASSERT_EQ(container.at(0).getPixel(), blackPixel) << std::to_string(idx);
    }
}
...
TEST_F(WorkerTest, CheckReflections){
    ...
    static auto accumulate = [](const SuperPixel& sPixel, const AveragePixel& averagePixel){
        const Pixel<> pixel = averagePixel.getPixel();
    ...
    EXPECT_GT(result.r, 1000 * 200 * factor);
    EXPECT_GT(result.g, 1000 * 100 * factor);
    EXPECT_GT(result.b, 1000 * 50 * factor);
    EXPECT_LT(result.r, 1000 * 255);
    EXPECT_LT(result.g, 1000 * 255);
    EXPECT_LT(result.b, 1000 * 255);
}